...
public class Controller
{
    ...
    {
        XElement xDB = XElement.Load($"http://syp.iis.nsk.su/SypWebApi/xml/tree2/{id}");
        XElement html = new XElement("div",
        xDB.Elements().Single(x => x.Name.LocalName == "text")
            .Elements().Single(x => x.Name.LocalName == "v").Value,
        xDB.Elements().Single(x => x.Attribute("pred").Value == "participant")
            .Elements().Select(studio =>
        {
            string studioId = studio.Elements().Single(x => x.Attribute("pred").Value == "in-org")
            .Elements().Single(x => x.Attribute("tp").Value == "org-sys")
                .Attribute("id")
                    .Value;

            string studioName = studio.Elements().Single(x => x.Attribute("pred").Value == "in-org")
                .Elements().Single(x => x.Attribute("tp").Value == "org-sys")
                .Elements().Single(x => x.Attribute("pred").Value == "name")
        ...
        xDB.Elements().Single(x => x.Attribute("pred").Value == "reflected")
            .Elements().Select(photo =>
            {
                string uri = photo.Elements().Single(x => x.Attribute("pred").Value == "in-doc")
                    .Elements().Single(x => x.Attribute("tp").Value == "photo-doc")
                    .Elements().Single(x => x.Attribute("pred").Value == "uri").Value;
    ...
    public static string CreateStudioHtml(string id)
    {
        XElement xDB = XElement.Load($"http://syp.iis.nsk.su/SypWebApi/xml/tree2/{id}");
        XElement html = new XElement("div",
        xDB.Elements().Single(x => x.Attribute("pred").Value == "name")
            .Elements().Single(x => x.Name.LocalName == "v").Value,
        xDB.Elements().Single(x => x.Attribute("pred").Value == "in-org")
            .Elements().Select(person =>
        {
            string personId = person.Elements().Single(x => x.Attribute("pred").Value == "participant")
            .Elements().Single(x => x.Attribute("tp").Value == "person")
                .Attribute("id")
                    .Value;

            string personName = person.Elements().Single(x => x.Attribute("pred").Value == "participant")
                .Elements().Single(x => x.Attribute("tp").Value == "person")
                .Elements().Single(x => x.Attribute("pred").Value == "name")
                .Elements().Single(x => x.Name.LocalName == "v")
                    .Value;
